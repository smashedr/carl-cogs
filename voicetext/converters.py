...
class CarlChannelConverter(IDConverter):
    ...
    CheckObjects = Union[discord.TextChannel, discord.CategoryChannel, discord.VoiceChannel, discord.StageChannel]

    async def convert(self, ctx: commands.Context, argument: str) -> CheckObjects:
        if not ctx.guild:
            raise errors.NoPrivateMessage()
...
# class CarlVoiceCategory(IDConverter):
#     """This is to convert ID's from a category, voice, or text channel via ID's or names"""
#
#     async def convert(self, ctx: commands.Context, argument: str) -> Union[discord.CategoryChannel, discord.VoiceChannel, discord.StageChannel]:
#         if not ctx.guild:
#             raise errors.NoPrivateMessage()
#
#         id_match = self._get_id_match(argument) or re.match(r"<#([0-9]+)>$", argument)
...
#
#
# class CarlCategoryConverter(IDConverter):
#     """This is to convert ID's from a category, voice, or text channel via ID's or names"""
#
